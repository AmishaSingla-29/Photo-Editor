...
onload = function () {
    ...
    // Tool is a map where a name is mapped to a function
    const tools = {
        "upload" : function () {
    ...
    for(let button of toolbar.children){
        // for each button we're adding an event to it
        if(button.nodeName==="BUTTON") {
            button.onclick = function (event) {
                event.preventDefault();
                tools[this.id].call(this);
                // this.id is name as in tools (see above at top) and call this means call the mapped function to name in tool
            }
        }
    }
